...
<p>
  This project is designed following the six pillars of the <strong>AWS Well-Architected Framework</strong>, ensuring a production-grade serverless application.
</p>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Infrastructure as Code (IaC):</strong> Entire stack provisioned via Terraform for consistent, repeatable deployments.</li>
          <li><strong>CI/CD Integration:</strong> Automated pipelines using GitHub Actions for linting, security scanning (Checkov), and deployment.</li>
          <li><strong>Observability:</strong> Centralized logging via Amazon CloudWatch to monitor Lambda execution and Polly API response status.</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Least Privilege:</strong> Custom IAM roles and policies grant Lambda only the specific permissions needed for S3 and Polly.</li>
          <li><strong>Data Protection:</strong> S3 buckets utilize Server-Side Encryption (SSE) to protect data at rest.</li>
          <li><strong>Secure Workflows:</strong> GitHub Actions uses OIDC (OpenID Connect) for AWS authentication, eliminating the need for long-lived access keys.</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Serverless Architecture:</strong> Utilizes managed services (S3, Lambda, Polly) that inherently provide high availability across multiple Availability Zones.</li>
          <li><strong>Event-Driven Design:</strong> S3 Event Notifications ensure the system automatically responds to new data without manual intervention.</li>
          <li><strong>Error Handling:</strong> Lambda function logic includes guardrails for character limits and processing failures.</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Right-Sizing:</strong> Serverless compute (Lambda) automatically scales based on the number of files uploaded</li>
          <li><strong>Optimized Storage:</strong> Dedicated Input and Output buckets prevent processing loops and optimize data flow.</li>
          <li><strong>Strategic Deployment:</strong> Resources are deployed in <code>us-east-1</code> to leverage the most robust and mature infrastructure environment in the AWS global network.</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Pay-as-you-go:</strong> No upfront costs; charges are only incurred during Lambda execution and Polly synthesis.</li>
          <li><strong>Regional Savings:</strong> Deployed in <code>us-east-1</code>, typically the most cost-effective region with the broadest Free Tier eligibility.</li>
          <li><strong>Lifecycle Management:</strong> S3 lifecycle rules automatically abort incomplete uploads and clean up old audio files after 30 days to save on storage.</li>
          <li><strong>Free Tier Friendly:</strong> Architecture fits well within the AWS Free Tier for small-to-medium scale usage.</li>
          ...
          <li><strong>Terraform Cloud Manual Apply:</strong> Prevents accidental resource creation that could incur costs.</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
          <li><strong>Managed Services:</strong> By using serverless resources, we maximize the utilization of underlying hardware, reducing the carbon footprint compared to idle EC2 instances.</li>
          <li><strong>Resource Clean-up:</strong> Automated deletion of temporary test artifacts in the CD pipeline ensures no wasted cloud resources.</li>
        </ul>
      </td>
    </tr>
  </tbody>
